;+
; tvSwitcher: A plug-in selector with hotkeys designed to work with a
;  tvdraw object.  
;-
...
pro tvSwitcher::Message, msg
  ...
  wh=where((*self.OList).keys eq char,cnt)
...
;=============================================================================
;       tvSwitcher_Event and Event.  Handle the internal hotkey events...
;=============================================================================
...
function tvSwitcher::Init,oDraw, USECASE=uc, SELECT_BASE=sb, _EXTRA=e
   ...
   self.Key->Signup,/ALL ;XXX really ALL?
      ...
      if widget_info(sb,/VALID) then begin 
         if widget_info(sb,/TYPE) eq 0 then begin ;it's a good base
            ...
            if obj_valid(objs[0]) then begin
               widget_control, sb, EVENT_PRO='tvSwitcher_Event', $
...
pro tvSwitcher__define
   struct={tvSwitcher, $ 
           ...
           oList: ptr_new(), $  ;a list of {obj:obj_new(), keys:''} structs
