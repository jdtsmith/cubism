...
;    all relevant objects on the "message recipient list" through each
;    objects' "Message" method -- a function meant to be overridden in
;    derived classes, and to catch the message and do useful things
;    with it.  A simple example might be a display status line object
;    which monitors draw window motion events to update a position
;    label.
;
;    ObjMsg enables an arbitrary but specified message-flow structure,
;    rather than just the "up-the-widget-tree" passing paradigm
;    implicit in normal widget programming.
;
; CATEGORY:
;
...
;             with methods MsgList and MsgRemove.
;
;    GetMsgObjs: 
;       
;       DESCRIPTION:
...
;          ala:
;          
;	      pro omInherit::MsgList, msgList
;                self->ObjMsg::MsgList, msgList
...
;             form (which can vary by sub-class), to be added to the
;             list.  Only one entry per object is allowed on the list,
;             and a newer entry will displace an older one.
;
;    MsgRemove: (override)
;       
;       DESCRIPTION:
...
;    om->MsgList,[om2,om3,om4,om5] ; add some to the message list
...
;=============================================================================
;      MsgList - Add an ObjMsg inherited object to the
;      message recipient list, or change existing list members.
;      Only one entry per object is allowed... if more than one entry
;      is passed for a given object, the last in the list is used. As
;      usual, the list elements can have any type, and the function GetObj
